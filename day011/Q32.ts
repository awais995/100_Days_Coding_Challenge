...
let registered_users: string [] = ["Ahmed", "rahman", "rafiu", "Taimoor"];
let new_user: string [] = ["Rahman", "Rafiu", "Ali", "Latif"];

// write the version that checks whether the new user name is already registered or not.
new_user.forEach(new_user=>{ // used loop and conditionals to handle case insensitive comparision.
// used .some() method to Determines whether the specified callback function returns true for any element of an array.    
    if(registered_users.some(registered_users => 
    registered_users.toLowerCase() === new_user.toLowerCase())){ 
